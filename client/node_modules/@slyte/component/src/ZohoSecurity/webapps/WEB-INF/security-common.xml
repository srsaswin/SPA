...
<security xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="security.xsd">
	<urls>
		...
		<url path="/zsecagentnotify" method="post" internal="true" scoped-services="AppSense,SOC" description="To enable/disable CSP log report">
		    <param name="data" type="JSONObject" template="zsecagentnotify_template" max-len="10000"/>
		</url>
	</urls>
   <jsontemplates>
     <jsontemplate name="zsecagentnotify_template">
        <key name="SERVICE" regex="servicename"/>
        <key name="COMPONENT" regex="servicename"/>
        <key name="DATA" type="JSONArray" template="zsecagentnotify_data_template" max-len="10000"/>
     </jsontemplate>
     <jsontemplate name="zsecagentnotify_data_template">
        <key name="STATUS" regex="^(enable|disable|ENABLE|DISABLE|RULE_ADDED|RULE_UPDATED|RULE_DELETED)$"/>
        <key name="RULE_AUTO_IDS" type="JSONArray" template="primitivelong"/>
        <key name="HASH" type="JSONArray" template="hashtemplate" max-len="10000" json-array-size="0-20"/>
     </jsontemplate>
     <jsontemplate name="primitivelong">
         <key index="0-10" type="long"/>
     </jsontemplate>
     <jsontemplate name="hashtemplate">
         <key index="0-20" regex="hexvalue" max-len="200"/>
     </jsontemplate>
   </jsontemplates>
	...
	<regexes>
		<regex name="accept_regex" value="^((,)?( )?([a-zA-Z*0-9+\/.-]*)(\s)*(;([a-zA-Z ]*)=([a-z0-9:\/&quot;.-]+))?)+$" />
		<regex name="accept_encoding_regex" value="^((,)?( )?(deflate|gzip|\*|compress|identity|gzip|sdch|br)(;q=[0-9].[0-9])?)+$" />
		<regex name="accept_language_regex" value="^((,)?( )?([a-zA-Z]{2}|\*)(-([a-zA-Z0-9]{1,5}))?(;q=[0-9.]{1,3})?)+$" />
		...
		<regex name="range_regex" value="^((bytes|,)?( )?([0-9]+-[0-9]+))+$" />
		...
		<regex name="websocket_extensions_regex" value="^((;)?( )?([A-Za-z0-9-_]+))+$" />
	...
	<regexes>
	    <regex name="iscsignature" value="[a-zA-Z0-9\-]+"/>
		<regex name="email" value="^[\w]([\w\-\.\+\'\/]*)@([\w\-\.]*)(\.[a-zA-Z]{2,22}(\.[a-zA-Z]{2}){0,2})$" />
		<!-- The username regex should not be used for creating new username -->
		<regex name="username" value="^[A-Za-z0-9_.]+$" />
		<regex name="username_or_email" value="^(([\w\.]+)|[\w]([\w\-\.\+\'\/]*)@([\w\.\-]+\.[a-zA-Z]{2,22}))$" />
		...
		<regex name="jsessionid" value="^[A-F0-9]{32}$" />
	...
	<xsspatterns>
		<xsspattern name="safehtmlfilter"
  				...
  				decode-flag="enable-all-decoders"
  ...
  />
		...
		<xsspattern name="htmlfilter" skip-neko-output-escaping="true"
			...
			insert-element-attributes="embed|allowscriptaccess|never,object|allowscriptaccess|never,iframe|allowfullscreen|false"
			trusted-script-domains="^([a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)*((google(((syndication|-analytics)?\.com)|(ads\.g\.doubleclick\.net)))|((baihui|zoho)\.com))$"
			decode-flag="enable-all-decoders" ignore-meta-charset="true" />
        ...
        <xsspattern name="cssfilter_raw_declarations" filter-css-only="true" filter-css-raw-declarations="true"
    	   ...
    	   remove-css-value="javascript|absolute|^expression"/>
		<!-- must be given for the input data to be shown part of json -->
		<xsspattern name="jsonfilter" extends="htmlfilter" />
	...
	<properties>
		<property name="http.ip.black.list.dns" value="httpbl.zoho1.com" />
		<property name="http.ip.white.list.dns" value="httpwl.zoho1.com" />
