...
worker.onmessage = function(event){
  ...
  if(obj && obj.lyteidb){
    if(obj.type == "open"){
      if(db.connector.application == undefined || !db.connector.application.differIDBAction){
        setTimeout(function(){
      ...
      var key = Object.keys(db.$.idbQ2)[0]
      var arr = obj.data || [];
      arr.forEach(function(item,index){
        var rec = db.cache.getEntity(key,item);
  ...
  if(Object.keys(db.$.idbQ2).length == 0){
    LyteIDB.isProcessing = false;
  }
};
window.addEventListener("load",function(){
  ...
  for(var key in db.schema){
    var pK = db.schema[key]._pK;
    models.push({modelName:key,pK:pK});
  }
  var scope = db.$.cbScp("app", "idbBeforeOpen"), args;
  if(scope){
    models = db.$.cB(scope, [models]);
  }
...
var LyteIDB = {
    ...
    IDBInsert:function(){
      ...
      if(!isProcessing && Object.keys(db.$.idbQ2).length){
        LyteIDB.isProcessing = true;
        var module;
        var module = Object.keys(db.$.idbQ2)[0];
        while(module && db.$.idbQ2[module] && db.$.idbQ2[module].length == 0){
          delete db.$.idbQ2[module];
          module = Object.keys(db.$.idbQ2)[0];
        }
        if(module){
            var q = db.$.idbQ2[module];
            var pK = db.getSchema(module)._pK;
            if(q.length){
